create database loan_Statements;
use loan_statements;

create table Loan_status(Loan_id varchar(40),Customer_id varchar(40),
Loan_Amount varchar(50),Loan_Amount_term int,Cibil_Score int,primary key(Loan_id));


create table Remark_update_det(Loan_id varchar(40),Loan_amount varchar(50),Cibil_score int,
Cibil_score_status varchar(50),primary key(Loan_id));

alter table loan_status modify Loan_amount varchar(50);
alter table Remark_update_det modify Loan_amount varchar(50);
Describe Loan_status;
describe remark_update_det;
select * from remark_update_det;
show tables;

Delimiter $$
create trigger Loan_status_checks1 after insert on Loan_status for each row
begin
if new.Cibil_score>900 then
insert into Remark_update_det(Loan_id,Loan_amount,cibil_score,cibil_score_status) values (new.Loan_id,new.Loan_amount,new.cibil_score,'High cibil score');

elseif new.Cibil_score>750 then
insert into Remark_update_det(Loan_id,Loan_amount,cibil_score,cibil_score_status) values (new.Loan_id,new.Loan_amount,new.cibil_score,'No penalty');

elseif new.Cibil_score>250 then
insert into Remark_update_det(Loan_id,Loan_amount,cibil_score,cibil_score_status) values (new.Loan_id,new.Loan_amount,new.cibil_score,'Penalty Customers');

elseif new.Cibil_score<=0 then
insert into Remark_update_det(Loan_id,Loan_amount,cibil_score,cibil_score_status) values (new.Loan_id,new.Loan_amount,new.cibil_score,'Reject Customers(Loan Cannot Apply)');
...
delimiter &&
create trigger remarks_detail1 before insert on Loan_status for each row
begin
...
create table Cibil_score_Status select Loan_status.Loan_id,Loan_status.Customer_id,Loan_status.Loan_amount,Loan_status.Loan_Amount_term,
Loan_status.cibil_score,remark_update_det.cibil_score_status 
from Loan_status inner join remark_update_det on Loan_status.Loan_id=remark_update_det.Loan_id;
...
-- task 1
create table Loan_Processing select  cibil_score_status.Loan_id,cibil_score_status.customer_id,cibil_score_status.Loan_amount,cibil_score_status.Loan_Amount_term,
cibil_score_status.cibil_score,cibil_score_status.cibil_score_status ,
 customer_income_status.ApplicantIncome,customer_income_status.coapplicantIncome,customer_income_status.property_Area,customer_income_status.Loan_Status ,
 customer_info.customer_name,customer_info.Gender,customer_info.age,customer_info.Married,customer_info.Education,customer_info.Self_employed,
country_state.region_id,country_state.postal_code,country_state.segment,country_state.state,
region_info.region from cibil_score_status 
inner join customer_income_status on cibil_score_status.Loan_id=customer_income_status.loan_id
inner join customer_info on customer_income_status.customer_id=customer_info.customer_id 
inner join country_state on customer_info.customer_id=country_state.customer_id 
inner join region_info on country_state.region_id=region_info.region_id;

select * from Loan_processing;


create table region_diff select country_state.customer_id,country_state.Loan_id,country_state.customer_name,country_state.region_id,country_state.postal_code,
country_state.segment,country_state.state,region_info.region
from country_state right  join region_info on country_state.region_id=region_info.region_id;


select * from region_diff;
...
select * ,
case
when ApplicantIncome>15000 then "A_Grade Customer"
when ApplicantIncome >9000 then "B_Grade Customer"
when ApplicantIncome >5000 then "Middle_Class_customer"
else "Low_Class Customer"
...
end as Lowclass_Monthy_interest_Calculation ,
(select round(Loan_amount*Lowclass_Monthy_interest_Calculation/100,0))as Monthly_interest ,
(select round(Loan_amount*Lowclass_Monthy_interest_Calculation*12,0))as Annual_interest 
from Loan_processing ;

select count(country_state.customer_name) ,State from Country_State left join region_info on Country_State.region_id=region_info.region_id group by Country_State.State;

select sum(Loan_amount),State from Loan_processing group by State; 

select sum(ApplicantIncome),state from Loan_processing group by state;

select customer_name,right(customer_name,4)as Customer_first_name from country_state where customer_name like "____%";
end //
delimiter ;
...
-- Update Gender for particular customer
...
update Customer_info set Gender = 'Female' where Customer_id =  "IP43593";
 ...
 select * from customer_info where customer_id = "IP43593";
select * from customer_info where customer_id = "IP43038";

 -- Update Age for particular Customer
 update Customer_info set Age = 45 where Customer_id =  "IP43007";
update Customer_info set Age = 32 where Customer_id =  "IP43009";
 
select * from customer_info where customer_id = "IP43007";
select * from customer_info where customer_id = "IP43009";
